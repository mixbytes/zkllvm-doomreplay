...
boolean PIT_StompThing (mobj_t* thing)
{
    ...
    if ( abss(thing->x - tmx) >= blockdist
	 || abss(thing->y - tmy) >= blockdist )
    {
	// didn't hit it
	return true;
    }
...
void P_HitSlideLine (line_t* ld)
{
    ...
    newlen = FixedMul (movelen, finesine[FINEANGLES/4 + deltaangle]);

    tmxmove = FixedMul (newlen, finesine[FINEANGLES/4 + lineangle]);	
...
boolean PTR_ShootTraverse (intercept_t* in)
{
	...
	dist = FixedMul (attackrange, in->frac);
        ...
        // backsector can be 0 when emulating missing back side.
  ...
  fixed_t	distance )
{
    ...
    x2 = t1->x + (distance>>FRACBITS)*finesine[FINEANGLES/4 + angle];
...
void P_UseLines (player_t*	player) 
{
    ...
    x1 = player->mo->x;
    y1 = player->mo->y;
    x2 = x1 + (USERANGE>>FRACBITS)*finesine[FINEANGLES/4 + angle];
...
boolean PIT_RadiusAttack (mobj_t* thing)
{
    ...
    dx = abss(thing->x - bombspot->x);
    dy = abss(thing->y - bombspot->y);
