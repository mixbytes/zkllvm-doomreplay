...
void A_Tracer (mobj_t* actor)
{
    ...
    actor->momx = FixedMul (actor->info->speed, finesine[FINEANGLES/4 + exact]);
...
void A_Fire (mobj_t* actor)
{
    ...
    actor->x = dest->x + FixedMul (24*FRACUNIT, finesine[FINEANGLES/4 + an]);
...
{	
    ...
    fire->x = actor->target->x - FixedMul (24*FRACUNIT, finesine[FINEANGLES/4 + an]);
...
void A_FatAttack1 (mobj_t* actor)
{
    ...
    mo->momx = FixedMul (mo->info->speed, finesine[FINEANGLES/4 + an]);
...
void A_SkullAttack (mobj_t* actor)
{
    ...
    A_FaceTarget (actor);
    an = actor->angle >> ANGLETOFINESHIFT;
    actor->momx = FixedMul (SKULLSPEED, finesine[FINEANGLES/4 + an]);
  ...
  angle_t	angle )
{
    ...
    x = actor->x + FixedMul (prestep, finesine[FINEANGLES/4 + an]);
